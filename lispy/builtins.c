...
{\
    BUILTIN_ARG_CHECK(a, 2, symbol, 2, a->cell_count);\
    lval* comp_a = lval_to_bool(lval_pop(a, 0));\
    lval* comp_b = lval_to_bool(lval_pop(a, 0));\
    lval_del(a);\
    truth_value_t comp_a_num = comp_a->truth_value;\
    truth_value_t comp_b_num = comp_b->truth_value;\
    lval_del(comp_a); lval_del(comp_b);\
    if (comp_a_num operator comp_b_num) return lval_bool(LVAL_TRUE);\
...
lval* builtin_equals(lenv* e, lval* a)
{
    ...
    lval* comp_a = lval_pop(a, 0);
    lval* comp_b = lval_pop(a, 0);
    lval* intermediate_result = lval_num(lvals_equal(comp_a, comp_b));
    lval* result = lval_to_bool(intermediate_result);
...
int lvals_equal(lval* comp_a, lval* comp_b)
{
    ...
    switch (comp_a->type) 
    {
        case LVAL_STR: return (strcmp(comp_a->str, comp_b->str) == 0);
